 ...
 * into ~/.pilotdeck/skills. User-owned skills are not overwritten; explicitly
 * repository-managed production contracts are refreshed from the repo.
 *
 * Behaviour:
 *   1. Every run: discover repo skills and copy missing slugs into
 *      $PILOT_HOME/skills, skipping existing user-owned targets and refreshing
 *      explicitly managed production contracts.
...
// Tongcheng's production handoff contract is repository-managed. It must track
// the checked-in version so a stale installed copy cannot silently run an old
// courseware protocol. Other skills remain user-owned once installed.
const MANAGED_REPO_SKILL_SLUGS = new Set(['tongcheng-courseware-handoff']);
...
function syncRepoSkillsToPilotHome(pilotHome) {
  ...
  if (skillDirs.length === 0) {
    return { created: 0, updatedManaged: 0, skippedExisting: 0, skippedDuplicateSlug: 0 };
  }
  ...
  for (const sourceDir of skillDirs) {
    ...
    if (existsSync(targetPath)) {
      if (MANAGED_REPO_SKILL_SLUGS.has(slug)) {
        try {
          cpSync(sourceDir, targetPath, { recursive: true, force: true });
          updatedManaged += 1;
        } catch (error) {
          console.warn(
            `[pilotdeck] Could not refresh managed repo skill '${slug}' at ${targetPath}: ${
  ...
  return { created, updatedManaged, skippedExisting, skippedDuplicateSlug };
}
...
function main() {
  ...
  if (skillSync.created > 0 || skillSync.updatedManaged > 0 || skillSync.skippedExisting > 0 || skillSync.skippedDuplicateSlug > 0) {
    console.log(
      `[pilotdeck] Synced repo skills into ${join(pilotHome, 'skills')}: ` +
        `${skillSync.created} copied, ${skillSync.updatedManaged} refreshed managed, ` +
        `${skillSync.skippedExisting} skipped existing, ` +
